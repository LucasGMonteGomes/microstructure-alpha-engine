...
namespace {
    ...
    std::string signalSideToString(SignalSide side) {
        switch (side) {
            case SignalSide::LONG:  return "LONG";
            case SignalSide::SHORT: return "SHORT";
    ...
    std::string breakoutPhaseToString(BreakoutPhase phase) {
        switch (phase) {
            case BreakoutPhase::IDLE:           return "IDLE";
            case BreakoutPhase::BREAK_UP:       return "BREAK_UP";
            case BreakoutPhase::BREAK_DOWN:     return "BREAK_DOWN";
            case BreakoutPhase::HOLD_UP:        return "HOLD_UP";
            case BreakoutPhase::HOLD_DOWN:      return "HOLD_DOWN";
            case BreakoutPhase::CONFIRMED_UP:   return "CONFIRMED_UP";
            case BreakoutPhase::CONFIRMED_DOWN: return "CONFIRMED_DOWN";
            case BreakoutPhase::FAILED:         return "FAILED";
...
// =============================================================================
// processSnapshot — núcleo do motor por snapshot recebido
// =============================================================================
...
void processSnapshot(
    ...
    // ------------------------------------------------------------------
    // Atualização dos componentes do motor
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Montagem do contexto e avaliação da estratégia
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Monitoramento e fechamento de posição aberta
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Filtros de persistência e qualidade de execução
    // ------------------------------------------------------------------
    ...
    signalDiagnosticsCollector.onSignal(
        snapshot, signal, regimeSnapshot,
        priceStructureSnapshot, breakoutState,
        persistenceApproved, executionApproved);
    ...
    // ------------------------------------------------------------------
    // Tentativa de abertura de posição
    // ------------------------------------------------------------------
...
int main() {
    ...
    BreakoutConfirmationV2Strategy strategy(strategyConfig);
    ...
    BreakoutStateEngine breakoutStateEngine(config);

    AggressionTracker aggressionTracker(config.aggressionWindowMs);
    SignalPersistenceFilter persistenceFilter(config.persistenceMinCount,
                                              config.persistenceMaxCount);
    RegimeFilter regimeFilter(config.regimeWindowMs);
    ...
    const std::int64_t testEndMs = testStartMs + config.testDurationMs;

    const TestRunPaths testRunPaths = TestRunPathsBuilder::build(config.testDurationMs);

    // ------------------------------------------------------------------
    // Coletores e escritores de resultado
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Estado compartilhado por ativo
    // ------------------------------------------------------------------
    ...
    binance_ws.setOnMessageCallback([&](const ix::WebSocketMessagePtr& msg) {
        ...
        try {
            ...
            // BookTicker
            if (data.contains("s") && data.contains("b") &&
                data.contains("a") && data.contains("B") && data.contains("A")) {
                ...
                processSnapshot(snapshot, config, strategy, executionQualityFilter,
                                paperTradeEngine, tradePublisher, tradeStatsCollector,
                                tradeCsvWriter, signalDiagnosticsCollector,
                                aggressionTracker, persistenceFilter, regimeFilter,
                                priceStructureEngine, breakoutStateEngine,
                                lastMidPriceByKey, lastSnapshotByKey);
                return;
            }

            // aggTrade
            if (data.contains("s") && data.contains("p") &&
                data.contains("q") && data.contains("m")) {
    ...
    bybit_ws.setOnMessageCallback([&](const ix::WebSocketMessagePtr& msg) {
        if (msg->type == ix::WebSocketMessageType::Open) {
            std::cout << "[REDE] Bybit conectada. Assinando streams..." << std::endl;
        ...
        try {
            ...
            if (topic.find("publicTrade.") == 0) {
                for (const auto& item : j["data"]) {
                    if (!item.contains("s") || !item.contains("p") ||
                        !item.contains("v") || !item.contains("S")) continue;
    ...
    // ==========================================================================
    // Fechamento forçado de posição aberta ao final do teste
    // ==========================================================================

    {
        ...
        if (paperTradeEngine.hasOpenPosition()) {
            ...
            if (it != lastSnapshotByKey.end()) {
                auto forcedCloseOpt = paperTradeEngine.forceClosePosition(
                    it->second, ExitReason::TIMEOUT);
